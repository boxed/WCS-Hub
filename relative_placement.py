...
class CoupleScoring(object):
    def __init__(self, name, judges_scores, chief_judge_score, user_data):
...
def print_final_tabulation(final_result):
    for tabulation, couple_scoring in final_result:
        print ''.join(['{:>3} ({:>3})'.format(*x) for x in tabulation]), couple_scoring.chief_judge_score, repr(couple_scoring.name), couple_scoring.user_data
